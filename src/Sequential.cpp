...
struct Graph
{
    ...
    void removeEdge(int64_t u, int64_t v)
    {
        ...
        auto it_u = find_if(adj[u].begin(), adj[u].end(),
                            [v](const pair<int64_t, int64_t> &e)
                            { return e.first == v; });
        if (it_u != adj[u].end())
            adj[u].erase(it_u);
        // Remove (v,u)
        auto it_v = find_if(adj[v].begin(), adj[v].end(),
                            [u](const pair<int64_t, int64_t> &e)
                            { return e.first == u; });
        if (it_v != adj[v].end())
            adj[v].erase(it_v);
    }
               ...
               vector<int64_t> &adj_weight, int64_t &num_vertices, int64_t &nedges, Graph &G)
{
    ...
    stringstream ss;
    ss << "Loaded graph with " << num_vertices << " vertices and " << nedges << " edges";
    PRINT(ss.str());
    cout << ss.str() << endl;
    return true;
}
...
int main()
{
    ...
    for (int64_t i = 0; i < min<int64_t>(10, num_vertices); ++i)
    {
        ss_term << i + 1 << "\t" << (T.dist[i] == INT64_MAX ? "INF" : to_string(T.dist[i]))
    ...
    if (out_file.is_open())
    {
        ...
        for (int64_t i = 0; i < num_vertices; ++i)
        {
            out_file << i + 1 << "\t" << (T.dist[i] == INT64_MAX ? "INF" : to_string(T.dist[i]))
                     << "\t\t" << (T.parent[i] == -1 ? "NONE" : to_string(T.parent[i] + 1)) << "\n";
        }
    }

    // update file
    string updateFile = "large_update.txt";
    ...
    for (int64_t i = 0; i < min<int64_t>(10, num_vertices); ++i)
    {
        ss_term_updated << i + 1 << "\t" << (T.dist[i] == INT64_MAX ? "INF" : to_string(T.dist[i]))
                        << "\t\t" << (T.parent[i] == -1 ? "NONE" : to_string(T.parent[i] + 1)) << "\n";
    }
