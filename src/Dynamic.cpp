...
bool loadGraph(const string& filename, vector<idx_t>& xadj, vector<idx_t>& adjncy, 
    ...
    if (!file.is_open()) {
        if (rank == 0) cerr << "Error: Could not open file: " << filename << endl;
        return false;
    }
    ...
    while (getline(file, line)) {
        ...
        idx_t weight = 1;
        if (!(iss >> from >> to)) {
            if (rank == 0) cerr << "Error: Invalid format in line " << line_count << ": " << line << endl;
            return false;
        }
        iss >> weight;

        if (from <= 0 || to <= 0) {
            if (rank == 0) cerr << "Error: Invalid vertex index in line " << line_count 
                                << ": from=" << from << ", to=" << to << endl;
            return false;
        }
        if (from == to) {
            if (rank == 0) cout << "Warning: Skipping self-loop in line " << line_count 
                                << ": " << from << " -> " << to << endl;
            continue;
        }
    ...
    if (xadj[nvtxs] != static_cast<idx_t>(adjncy.size())) {
        if (rank == 0) cerr << "Error: CSR format invalid: xadj[" << nvtxs << "]=" << xadj[nvtxs] 
                            << ", adjncy.size()=" << adjncy.size() << endl;
        return false;
    }

    if (rank == 0) cout << "Loaded graph with " << nvtxs << " vertices and " << nedges << " edges" << endl;
    return true;
}
...
void partitionGraph(vector<idx_t>& xadj, vector<idx_t>& adjncy, 
    ...
    if (ret != METIS_OK) {
        if (rank == 0) cerr << "METIS failed with code: " << ret << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    parts.assign(part_idx.begin(), part_idx.end());
    if (rank == 0) cout << "METIS succeeded. Edge cut: " << objval << endl;
}
...
vector<pair<pair<int, int>, int>> generateChanges(const Graph& G, int num_changes, double insert_ratio, int rank) {
    ...
    random_device rd;
    mt19937 gen(rd() + rank); // Different seeds per rank
    ...
    // Generate random insertions or deletions
...
void parallelSSSPUpdate(Graph& G, SSSPTree& T, const vector<pair<pair<int, int>, int>>& changes, 
    ...
    for (int v = 0; v < V; ++v) {
        if (part[v] == rank) local_vertices.push_back(v);
    }
    ...
    for (size_t i = 0; i < changes.size(); ++i) {
        ...
        #pragma omp critical
        {
            if (w >= 0) { // Insertion (Algorithm 2, lines 11-20)
                G.addEdge(u, v, w);
            } else { // Deletion (Algorithm 2, lines 4-10)
                G.removeEdge(u, v);
            }
        }
        
        if (w >= 0) { // Handle edge insertion (Algorithm 2, lines 12-19)
        ...
        } else { // Handle edge deletion (Algorithm 2, lines 5-9)
    ...
    while (global_change) { // Continue until no changes (Algorithm 4, line 4)
        global_change = false;
        ...
        for (size_t i = 0; i < local_vertices.size(); ++i) {
            ...
            if (T.affected_del[v]) { // Check AffectedDel flag (Algorithm 4, line 8)
        ...
        for (size_t i = 0; i < local_vertices.size(); ++i) {
            int v = local_vertices[i];
            if (T.affected[v]) { // Check Affected flag (Algorithm 4, line 25)
...
int main(int argc, char* argv[]) {
    ...
    vector<int> send_buf(num_changes * 3);
    for (int i = 0; i < num_changes; ++i) {
        send_buf[i * 3] = changes[i].first.first;
        send_buf[i * 3 + 1] = changes[i].first.second;
        send_buf[i * 3 + 2] = changes[i].second;
    }
    vector<int> recv_buf(num_changes * 3 * size);
    MPI_Allgather(send_buf.data(), num_changes * 3, MPI_INT, 
                  recv_buf.data(), num_changes * 3, MPI_INT, MPI_COMM_WORLD);
    
    // Combine changes from all processes
    changes.clear();
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < num_changes; ++j) {
            int u = recv_buf[i * num_changes * 3 + j * 3];
            int v = recv_buf[i * num_changes * 3 + j * 3 + 1];
            int w = recv_buf[i * num_changes * 3 + j * 3 + 2];
            changes.push_back({{u, v}, w});
        }
    }
